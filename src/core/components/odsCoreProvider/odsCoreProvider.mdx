...
all the ODS components, defaulting such values to `<img />`, `<a />` tags, and a default set of copy texts. The provider
is used by the `<AvatarBase />` and `<LinkBase />` components, which are in turn used by all the other core and module
components.
