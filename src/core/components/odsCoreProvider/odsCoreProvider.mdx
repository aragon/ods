...
**Supported locale usage example**

```tsx
import { OdsCoreProvider } from '@aragon-ods';

const coreProviderValues = { locale: 'es' };
...
**Unsupported locale usage example**

```tsx
import { OdsCoreProvider, IOdsCoreCopy } from '@aragon-ods';

const byCopy: IOdsCoreCopy = {
    inputOptionalLabel: 'Неабавязкова',
};

const coreProviderValues = { locale: 'by', copy: byCopy };
